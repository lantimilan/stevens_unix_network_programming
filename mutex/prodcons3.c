// prodcons3.c
// gcc -o prodcons3 prodcons3.o error.o -lpthread
#include "unpipc.h"
...
consume_wait(int i)
{
  for ( ; ; ) {
    pthread_mutex_lock(&shared.mutex);
    if (i < shared.nput) { // item already produced
      // we have produced 0, 1, 2, ..., nput-1
      pthread_mutex_unlock(&shared.mutex);
      return; 
    }
    pthread_mutex_unlock(&shared.mutex);
...
consume(void *arg)
{
  ...
  for (i = 0; i < nitems; ++i) {
    consume_wait(i);  // wait for item to be produced
